...
/**
 * @author Ahmad Hamouda on 2/25/17.
 */
...
@ActiveProfiles("test")
@Transactional
public class UrlControllerTest extends AbstractJSONRestTest {
    private static final Logger LOG = Logger.getLogger(UrlControllerTest.class);
    ...
    public void testShortenUrl() {
        try {
            ...
            mockMvc.perform(post("/user/shortUrl/").param("url", setUp.getLongUrl())).
                    ...
                    andExpect(content().string(setUp.getShortUrl(setUp.getLongUrl())));
        } catch (Exception ex) {
            LOG.error(ex);
            fail(ex.getMessage());
        }
    ...
    public void testExpiredTimeUrl() {
        try {
            LOG.info("Test redirect url");
            Url url=setUp.createDateExpiredUrl();
            LOG.debug("URL is: "+setUp.getLongUrl());
            mockMvc.perform(get("/r/"+url.getShortenUrl())).
    ...
    public void testExpiredHitsUrl() {
        try {
            LOG.info("Test redirect url");
            Url url=setUp.createExpiredHitUrl();
            LOG.debug("URL is: "+setUp.getLongUrl());
            mockMvc.perform(get("/r/"+url.getShortenUrl())).
                    andExpect(status().isGone());
        } catch (Exception ex) {
